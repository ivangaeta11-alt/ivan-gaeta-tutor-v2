import React from "react";
import { Download } from "lucide-react";
import type { MaterialAssignment } from "../types";
import MaterialStatusBadge from "./MaterialStatusBadge";
import { formatDateShort } from "../../studente/utils/format";
import { useMaterials } from "../context/MaterialsContext";
import { getWorkspaceById } from "../data/materialsMock";

interface TutorAssignmentModalProps {
  assignment: MaterialAssignment;
  onClose: () => void;
}

const TutorAssignmentModal: React.FC<TutorAssignmentModalProps> = ({
  assignment,
  onClose,
}) => {
  const { navigateToLocation, folders } = useMaterials();
  const workspace = getWorkspaceById(assignment.workspaceId);

  const openInWorkspace = () => {
    const sub = folders.find(
      (f) =>
        f.workspaceId === assignment.workspaceId && f.systemKind === "submissions"
    );
    if (sub) {
      navigateToLocation(assignment.workspaceId, sub.id);
    }
    onClose();
  };

  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-slate-900/40">
      <div
        className="w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 bg-white rounded-3xl border border-slate-100 shadow-xl min-w-0"
        role="dialog"
        aria-modal="true"
      >
        <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
          <div className="min-w-0">
            <h2 className="text-lg font-bold text-slate-900 break-words">{assignment.title}</h2>
            <p className="text-sm text-slate-500 mt-1">
              {workspace?.name ?? assignment.subject} · {assignment.tutorName}
            </p>
          </div>
          <MaterialStatusBadge status={assignment.status} />
        </div>

        <section className="mb-4">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">
            Istruzioni
          </h3>
          <p className="text-sm text-slate-600 font-light break-words">{assignment.instructions}</p>
          <p className="text-xs text-slate-400 mt-2">
            Pubblicata il {formatDateShort(assignment.publishedAt)}
            {assignment.dueDate && ` · Scadenza: ${formatDateShort(assignment.dueDate)}`}
          </p>
        </section>

        {assignment.tutorAttachments.length > 0 && (
          <section className="mb-4">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">
              Allegati
            </h3>
            <ul className="space-y-2">
              {assignment.tutorAttachments.map((att) => (
                <li key={att.id}>
                  <button
                    type="button"
                    onClick={() => window.alert(`Download simulato: ${att.name}`)}
                    className="inline-flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-700"
                  >
                    <Download className="w-4 h-4 shrink-0" />
                    {att.name}
                  </button>
                </li>
              ))}
            </ul>
          </section>
        )}

        <section className="mb-6">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">
            Svolgimento studente
          </h3>
          {assignment.studentFile ? (
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 text-sm">
              <p className="font-medium text-slate-800">{assignment.studentFile.name}</p>
              <p className="text-xs text-slate-400 mt-1">
                Caricata il {formatDateShort(assignment.studentFile.uploadedAt)}
              </p>
              <button
                type="button"
                onClick={() =>
                  window.alert(`Download simulato: ${assignment.studentFile!.name}`)
                }
                className="mt-2 inline-flex items-center gap-1.5 text-sm font-semibold text-blue-600"
              >
                <Download className="w-4 h-4" /> Scarica
              </button>
            </div>
          ) : (
            <p className="text-sm text-slate-400 font-light">
              Nessuno svolgimento caricato dagli studenti.
            </p>
          )}
          {assignment.studentComment && (
            <p className="text-sm text-slate-500 mt-2 font-light break-words">
              Commento studente: {assignment.studentComment}
            </p>
          )}
        </section>

        <div className="flex flex-wrap justify-end gap-2">
          <button
            type="button"
            onClick={openInWorkspace}
            className="px-4 py-2 text-sm font-semibold rounded-xl border border-slate-200 text-slate-700 hover:border-blue-200"
          >
            Apri workspace
          </button>
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 text-sm font-semibold rounded-xl border border-slate-200 text-slate-600"
          >
            Chiudi
          </button>
        </div>
      </div>
    </div>
  );
};

export default TutorAssignmentModal;
